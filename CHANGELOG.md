...
- clear the cached Responses request shape on thinking-level changes too, so compacting immediately after switching thinking level no longer sends the previous level's `reasoning` shape; the next request at the new level repopulates the cache
