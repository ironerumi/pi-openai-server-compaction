...
- invalidate the cached Responses `reasoning` config on thinking-level changes, so compacting immediately after switching thinking level no longer sends the previous level's `reasoning` shape; the level-independent `text` config stays cached and the next request repopulates `reasoning`
- stop degrading `max` thinking requests to no reasoning: reasoning params are now built with Pi AI's `clampThinkingLevel` plus the model's `thinkingLevelMap`, so `max` clamps to the model's highest mapped tier (or passes through natively), and the off case follows each model's `thinkingLevelMap.off` (omitting `reasoning` when it is `null`) while GitHub Copilot still never receives an explicit off effort
