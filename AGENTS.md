...
- The cached Responses request shape (`getResponsesRequestShapeState`/`setResponsesRequestShapeState` in `src/state.ts`) records the last provider request's `reasoning`/`text` per session, and `session_before_compact` prefers it over the current-level fallback. It is keyed only by sessionId but is only valid for the model and thinking level that produced it, so both `model_select` and `thinking_level_select` in `src/index.ts` must clear it; a new invalidation trigger needs the same clear.
